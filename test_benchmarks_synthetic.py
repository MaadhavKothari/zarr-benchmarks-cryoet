    ...
    for codec in codecs:
        ...
        try:
            ...
            read_write_zarr.write_zarr_array(
                data, store_path, overwrite=False,
                chunks=(chunk_size, chunk_size, chunk_size),
                compressor=compressor, zarr_spec=2
            )
            ...
            ratio = 1.0 if codec == 'no_compression' else read_write_zarr.get_compression_ratio(store_path)
    ...
    for version in [2, 3]:
        ...
        try:
            ...
            read_write_zarr.write_zarr_array(
                data, store_path, overwrite=False,
                chunks=(chunk_size, chunk_size, chunk_size),
                compressor=compressor, zarr_spec=version
            )
            write_time = time.time() - t0
...
def test_data_integrity(data: np.ndarray, output_dir: pathlib.Path):
    ...
    for codec in codecs:
        ...
        try:
            ...
            print(f"✓ Exact:{is_exact} SSIM:{ssim_val:.6f} PSNR:{psnr_val:.1f}dB MSE:{mse_val:.2e} Corr:{correlation:.6f}")
