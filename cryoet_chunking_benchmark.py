...
for chunk_size in chunk_sizes:
    ...
    print(f"      Storage: {result['storage_mb']:.2f} MB, Ratio: {result['compression_ratio']:.2f}x")
...
df_display = df[[
    'name', 'chunks_shape', 'write_time', 'read_time_full',
    'read_time_chunk', 'read_time_slice', 'chunk_count',
    'file_count', 'storage_mb', 'compression_ratio'
]].round(4)
...
for _, row in cubic_data.iterrows():
    print(f"   {row['chunks_shape']:8s}: {row['chunk_count']:4.0f} chunks, "
...
print(f"   Fewest files: {df.loc[df['file_count'].idxmin(), 'name']} "
      f"({df['file_count'].min():.0f} files)")
print(f"   Best compression: {df.loc[df['compression_ratio'].idxmax(), 'name']} "
...
ax.plot(cubic_only['chunk_size'], cubic_only['write_time'],
...
width = 0.25
ax.bar([i - width for i in x], cubic_only['read_time_full'],
       width, label='Full Read', color='coral')
ax.bar(x, cubic_only['read_time_chunk'],
       width, label='Single Chunk', color='lightgreen')
ax.bar([i + width for i in x], cubic_only['read_time_slice'],
       width, label='Single Slice', color='plum')
...
ax = axes[0, 2]
ax.semilogy(cubic_only['chunk_size'], cubic_only['chunk_count'],
            'o-', linewidth=2, markersize=10, color='darkgreen', label='Data Chunks')
ax.semilogy(cubic_only['chunk_size'], cubic_only['file_count'],
            's--', linewidth=2, markersize=8, color='darkred', label='Total Files')
ax.set_xlabel('Chunk Size (voxels per dimension)')
...
ax = axes[1, 2]
ax.scatter(cubic_only['chunk_count'], cubic_only['read_time_chunk'] * 1000,
          s=200, alpha=0.6, c=cubic_only['chunk_size'], cmap='viridis')
...
print(f"   Result: ~{cubic_data[cubic_data['chunk_size'] == 64].iloc[0]['chunk_count']:.0f} chunks for 128³ volume")
...
print(f"    Example: 64³ chunks = ~{cubic_data[cubic_data['chunk_size'] == 64].iloc[0]['chunk_count']:.0f} files")
