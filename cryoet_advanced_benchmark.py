...
print(f"   This will take ~{len(CHUNK_CONFIGS) * len(COMPRESSION_CONFIGS) * N_RUNS * 2 // 60} minutes")
...
fig.suptitle('Advanced CryoET Benchmarks - Statistical Validation',
...
for idx, comp_name in enumerate(['blosc_zstd_5', 'blosc_lz4_3', 'no_compression']):
    ...
    x = range(len(df_comp))
    ax.errorbar(x, df_comp['write_mean'],
               yerr=[df_comp['write_mean'] - df_comp['write_min'],
                     df_comp['write_max'] - df_comp['write_mean']],
               fmt='o-', capsize=5, capthick=2, label=comp_name)
    ...
    ax.errorbar(x, df_comp['read_full_mean'],
               yerr=[df_comp['read_full_mean'] - df_comp['read_full_min'],
    ...
    ax.bar(x_pos - width/2, df_comp['slice_sequential_mean'] * 1000,
          width, label='Sequential', alpha=0.8)
    ax.bar(x_pos + width/2, df_comp['slice_concurrent_mean'] * 1000,
          width, label=f'Concurrent ({N_THREADS}T)', alpha=0.8)
...
for comp_name in df['compression'].unique():
    df_comp = df[df['compression'] == comp_name]
    ax.plot(df_comp['chunk_label'], df_comp['slice_speedup'],
           'o-', label=comp_name, markersize=8)
...
for comp_name in df['compression'].unique():
    df_comp = df[df['compression'] == comp_name]
    ax.errorbar(df_comp['chunk_label'], df_comp['random_mean'] * 1000,
               yerr=df_comp['random_std'] * 1000,
               fmt='o-', capsize=5, label=comp_name)
ax.set_xlabel('Chunk Configuration')
...
ax = axes[1, 0]
scatter = ax.scatter(df['file_count'], df['read_full_mean'] * 1000,
                    c=df['compression_ratio'], s=100, alpha=0.6, cmap='viridis')
...
for comp_name in df['compression'].unique():
    df_comp = df[df['compression'] == comp_name]
    ax.scatter(df_comp['compression_ratio'], df_comp['write_mean'],
              s=df_comp['file_count'], alpha=0.6, label=comp_name)
...
print(f"   {'Low variability = consistent performance' if (df['write_std'] / df['write_mean']).mean() < 0.1 else 'High variability detected'}")
